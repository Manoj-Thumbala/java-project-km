	...
	public AdminPanel() {
		...
		btnAddCustomer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					boolean added = DatabaseOperations.addCustomer(customerNameField.getText(), phoneField.getText(), emailField.getText(), addressField.getText());
					if (added) {
						JOptionPane.showMessageDialog(btnAddCustomer, "Added Successfully");
					} else {
						JOptionPane.showMessageDialog(btnAddCustomer, "Customer already exists");
					}
		...
		btnDeletecust.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					JOptionPane.showMessageDialog(btnDeletecust, "Customer Deleted Successfully");
					DatabaseOperations.loadData((DefaultTableModel) customerTable.getModel(), "customers");
				} catch (NumberFormatException e1) {
					JOptionPane.showMessageDialog(btnDeletecust, "Enter numeric value");
					e1.printStackTrace();
				} catch (SQLException e1) {
		...
		lblStock.setBounds(0, 174, 173, 87);
		addProductPanel.add(lblStock);

		stockField = new JTextField();
		stockField.setColumns(10);
		stockField.setBounds(173, 174, 173, 87);
		...
		btnAddProduct.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					boolean added = DatabaseOperations.addProduct(productNameField.getText(), Float.valueOf(priceField.getText()), Integer.valueOf(stockField.getText()));
					if (added) {
						JOptionPane.showMessageDialog(btnAddProduct, "Product Added Successfully");
					} else {
						JOptionPane.showMessageDialog(btnAddProduct, "Product already exists, stock updated");
					}
				} catch (NumberFormatException e1) {
					JOptionPane.showMessageDialog(btnAddProduct, "Enter valid numeric values");
					e1.printStackTrace();
				} catch (SQLException e1) {
					JOptionPane.showMessageDialog(btnAddProduct, "Can't Add Product");
					e1.printStackTrace();
				}
			}
		});
		btnAddProduct.setBounds(0, 261, 340, 87);
		...
		btnDeleteProduct.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					JOptionPane.showMessageDialog(btnDeleteProduct, "Product Deleted Successfully");
